...
import { MapPin, Phone, User, Hash, Globe, Calendar as CalendarIcon } from 'lucide-react';
import {
  Popover,
  PopoverContent,
  PopoverTrigger,
...
const clientSchema = z.object({
  ...
  client_code: z.string()
    .optional()
    .refine((val) => !val || /^[A-Z0-9-]+$/.test(val.toUpperCase()), {
      message: 'client.validation.clientCodeInvalid',
    }),
  full_name: z
    .string()
  ...
  const onSubmit = async (data: ClientFormData) => {
    ...
    try {
      ...
      if (data.referrer_telegram_id) {
        requestData.referrer_telegram_id = parseInt(data.referrer_telegram_id, 10);
      }
      if (data.referrer_client_code) {
        requestData.referrer_client_code = data.referrer_client_code.trim().toUpperCase();
      }
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    {form.formState.errors.telegram_id && (
                      <p className="text-red-500 text-sm">{t(form.formState.errors.telegram_id.message as string)}</p>
                    )}
                  </FormItem>
                )}
              />
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel className="text-gray-700 font-medium flex items-center gap-2">
                      ...
                      {t('client.fullName')} <span className="text-red-500">*</span>
                    </FormLabel>
                    <FormControl>
                      <Input
                        {...field}
                    ...
                    {form.formState.errors.full_name && (
                      <p className="text-red-500 text-sm">{t(form.formState.errors.full_name.message as string)}</p>
                    )}
                  </FormItem>
                )}
              />
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    {form.formState.errors.passport_series && (
                      <p className="text-red-500 text-sm">{t(form.formState.errors.passport_series.message as string)}</p>
                    )}
                  </FormItem>
                )}
              />
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    {form.formState.errors.pinfl && (
                      <p className="text-red-500 text-sm">{t(form.formState.errors.pinfl.message as string)}</p>
                    )}
                  </FormItem>
                )}
              />
                ...
                render={({ field }) => (
                  <FormItem className="flex flex-col">
                    ...
                    {form.formState.errors.date_of_birth && (
                      <p className="text-red-500 text-sm">{t(form.formState.errors.date_of_birth.message as string)}</p>
                    )}
                  </FormItem>
                )}
              />
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    {form.formState.errors.address && (
                      <p className="text-red-500 text-sm">{t(form.formState.errors.address.message as string)}</p>
                    )}
                  </FormItem>
                )}
              />
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    {form.formState.errors.phone && (
                      <p className="text-red-500 text-sm">{t(form.formState.errors.phone.message as string)}</p>
                    )}
                  </FormItem>
                )}
              />
                ...
                render={() => (
                  <FormItem>
                    ...
                    {form.formState.errors.passportImages && (
                      <p className="text-red-500 text-sm">{t(form.formState.errors.passportImages.message as string)}</p>
                    )}
                  </FormItem>
                )}
              />
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    {form.formState.errors.referrer_telegram_id && (
                      <p className="text-red-500 text-sm">{t(form.formState.errors.referrer_telegram_id.message as string)}</p>
                    )}
                  </FormItem>
                )}
              />
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    {form.formState.errors.referrer_client_code && (
                      <p className="text-red-500 text-sm">{t(form.formState.errors.referrer_client_code.message as string)}</p>
                    )}
                  </FormItem>
                )}
              />

              {/* Client Code (Editable in edit mode) */}
              {mode === 'edit' && (
                <FormField
                  control={form.control}
                  name="client_code"
                  render={({ field }) => (
                    <FormItem>
                      ...
                      {form.formState.errors.client_code && (
                        <p className="text-red-500 text-sm">{t(form.formState.errors.client_code.message as string)}</p>
                      )}
                    </FormItem>
                  )}
                />
